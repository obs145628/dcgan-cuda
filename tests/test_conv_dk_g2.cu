...
int main(int argc, char** argv)
{
    ...
    auto y_node = builder.input(ops::Shape({BATCH, 32 * 32 * 64}));
    const int strides[2] = {2, 2};

    const int out_size[4] = {BATCH, 32, 32, 64};
    ops::Op* yh_node = builder.conv2d_transpose(x_node, w_node, out_size, strides);;
    yh_node = builder.reshape(yh_node, ops::Shape({BATCH, 32 * 32 * 64}));
    ...
    graph.run({dw_node},
	      {
                  ...
                  {y_node, {y, ops::Shape({BATCH, 32 * 32 * 64})}},
              },
	      {dw_out});
